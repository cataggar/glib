...
//
// glibc ships working iconv() and gettext()/dgettext()/bindtextdomain()
// as part of libc itself. Windows (mingw) needs vendored libiconv and
// libintl implementations, while musl and macOS need vendored libintl;
// macOS gets iconv from its SDK.
...
const glib_linux_sources = [_][]const u8{
...
// dgettext()/bindtextdomain() on Windows, musl, and macOS -- see that
// package's build.zig for details. Its
// localcharset.c is likewise excluded for the same reason.
const gettext_intl_sources = [_][]const u8{
    "bindtextdom.c",
    "dcgettext.c",
    "dgettext.c",
    "gettext.c",
...
pub fn build(b: *std.Build) void {
    ...
    const is_musl = is_linux and target.result.abi == .musl;
    const needs_libintl = is_windows or is_musl or is_macos;
    const is_glibc = is_linux and !is_musl;
    ...
    // below cover 64-bit little-endian Linux/macOS (LP64) targets and a
    // 64-bit Windows/mingw (LLP64) target, selected by target compiler
    // macros.
    ...
    _ = write_files.add("config.h",
        ...
        \\// Windows, and macOS link the Zig gettext package's libintl
        \\// sources.
        \\#define ENABLE_NLS 1
        ...
        \\// Portable POSIX headers used by the core sources.
        \\#ifndef _WIN32
    ...
    if (needs_libintl) {
        ...
        intl_mod.addCSourceFiles(.{
            ...
            .files = &gettext_intl_sources,
            .flags = &.{ "-std=gnu99", "-w" },
        });
        // libgnuintl.in.h redirects plain setlocale() to libintl_setlocale()
        // on Windows and macOS, and glib's own gdatetime.c calls setlocale().
        // See cataggar/gettext's setlocale-compat.c for the full rationale.
